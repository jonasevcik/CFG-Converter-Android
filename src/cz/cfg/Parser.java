...
public class Parser {
    ...
    public List<String> orderingOfNonTerminals(String input) {
    ...
    public ContextFreeGrammar parse(String input) throws ParserException {
        ...
        for (int i = 0; i < arrayRules.length; i++) {
            ...
            if (actualRule.split("->").length != 2) {
                throw new ParserException("Chyba syntaxe: Nebyla uvedena šipka (->), popř. chybí pravá strana pravidla.");
            ...
            if ((leftSide.replaceAll("\\\\e", "").length() == 1 && Character.isUpperCase(leftSide.charAt(0))) || (leftSide.split("<").length == 2) || (leftSide.contains("\'") && leftSide.split("'").length == 1)) {
                ...
                for (int j = 0; j < rightSideRule.length; j++) {
                    if (rightSideRule[j].equals("\\\\e")) {
                        ...
                        for (int k = 0; k < rightSideRule[j].length(); k++) {
                            ...
                            if ((next == null || !next.equals(apostrof)) && !symbol.toString().startsWith("<")) { //pokud je neterminĂˇl
                                if (Character.isUpperCase(ch)) {
                                    list.add(ch.toString());
                                    nonTerminals.add(ch.toString());
                                    symbol.delete(0, symbol.length()); //??? ano ???
                                } else if (!ch.equals(apostrof) && (next == null || !next.equals(apostrof))) { //pokud je terminĂˇl
                                    list.add(ch.toString());
                            ...
                            if (symbol.toString().startsWith("<") && symbol.toString().endsWith(">")) { //pokud je neterminĂˇl tvaru <nÄ›co>
                                list.add(symbol.toString());
                                nonTerminals.add(symbol.toString());
                                symbol.delete(0, symbol.length());
                            }
            ...
            } else {//pokud nebyla bezkontextovĂˇ
                ...
                for (int k = 0; k < leftSide.length(); k++) {
                    ...
                    if ((symbol.toString().startsWith("<") && k == leftSide.length() - 1 && !symbol.toString().endsWith(">")) || (!symbol.toString().startsWith("<") && k == leftSide.length() - 1 && symbol.toString().endsWith(">"))) {
                        throw new ParserException("Chyba syntaxe: Neterminál v závorkách není správnš uzavřen.");
                    }
                }
                ...
                //levou stranu pravidla mĂˇme zpracovanou
                ...
                for (int j = 0; j < rightSideRule.length; j++) {
                    ...
                    if (rightSideRule[j].equals("\\\\e")) {
                        ...
                        for (int k = 0; k < rightSideRule[j].length(); k++) {
                            ...
                            if ((next == null || !next.equals(apostrof)) && !symbol2.toString().startsWith("<")) { //pokud je neterminĂˇl
                                if (Character.isUpperCase(ch)) {
                                    rightHelpList.add(ch.toString());
                                    symbol2.delete(0, symbol.length()); //??? ano ???
                                } else {                                        //pokud je terminĂˇl
                                    rightHelpList.add(ch.toString());
                                    symbol2.delete(0, symbol.length()); //??? ano ???
                                }
                            }
                            if (symbol2.toString().startsWith("<") && symbol2.toString().endsWith(">")) { //pokud je neterminĂˇl tvaru <nÄ›co>
                                rightHelpList.add(symbol2.toString());
                                symbol2.delete(0, symbol2.length());
                            }
